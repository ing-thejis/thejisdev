...
function SkillIcon({ skill }: { skill: Skill }) {
  ...
  return (
    <span className="text-base leading-none flex-shrink-0" style={{ width: 20, textAlign: 'center' }}>
      {skill.icon}
    </span>
  )
}
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className={`mb-16 transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <p className="section-subtitle max-w-xl">
            Technologies and tools I use to build modern, scalable, AI-powered applications.
          </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {skillCategories.map((category, catIdx) => (
            <div
              key={category.title}
              className={`card p-6 transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
        ...
        {/* Footer note */}
        <p
          className={`text-center text-sm mt-12 font-mono transition-all duration-700 delay-500 ${visible ? 'opacity-100' : 'opacity-0'}`}
          style={{ color: '#52525b' }}
        >
